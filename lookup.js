  ...
  function setupAutoFill(){
    function attach(){
      document.querySelectorAll('input').forEach(function(input){
        ...
        const isMem = ph.includes('عضوية') || input.id?.toLowerCase().includes('membership');
        if(isMem || input.type === 'number' || (input.closest('label')?.textContent||'').includes('العضوية')){
          ...
          input.addEventListener('blur', async function(){
            ...
            if(branch){
              ...
              document.querySelectorAll('input, select, div').forEach(function(el){
                ...
                if(txt.includes('من فرع') || txt.includes('فرع العضوية')){
