...
(function () {
  ...
  function loadApp() {
    var texts = window.WD_TEXTS;
     var appUrl = new URL("app.js", document.baseURI).href;
     if (!texts) return injectApp(appUrl);
     fetch(appUrl)
      .then(function (r) { return r.text(); })
      .then(function (code) {
        Object.keys(texts).forEach(function (orig) {
          if (texts[orig] === orig) return;
          var out = esc(texts[orig]);
          code = code
            .replace(new RegExp('"' + rx(orig) + '"', "g"), out)
            .replace(new RegExp("'" + rx(orig) + "'", "g"), out);
        });
        injectApp(URL.createObjectURL(new Blob([code], { type: "text/javascript" })));
      })
       .catch(function () { injectApp(appUrl); });
  }
